...
const userSignup = (req, res, next) => {
    ...
    if (!cpf.match(cpfcnpjRegex)) {
        return res.status(400).send({
            errors: ["O CPF ou CNPJ informado está inválido"]
        })
    }
    
    /*if (!cep.match(cepRegex)) {
        return res.status(400).send({
            errors: ["O CEP informado está inválido"]
        })
    }*/
    ...
    User.findOne({ email }, (err, user) => {
        if (err) {
            ...
            const newUser = new User({ name, email, password: passwordHash, cpf,cep })
            newUser.save(err => {
                if (err) {
                    return sendErrorsFromDB(res, err)
                } else {
...
module.exports = { login, userSignup, validateToken }